#!/usr/bin/env python3
"""WP6b fixed-K register-state representation comparison."""
...
class RecurrentRegisterStateTransformer(nn.Module):
    def __init__(
        self,
        *,
        d_model: int = 64,
        ...
        # Preserve frozen direct initialization order before control-only state.
    ...
    def forward(
        ...
        *,
        ...
        for step in range(recurrent_steps):
            ...
            if treatment == "latent":
                slot = (
                    hidden[batch, cls_index].unsqueeze(1)
                    if step == 0
                    else hidden[:, -1:, :]
                )
            else:
        ...
        hidden = self._encode(current, padding_mask)
        calls += 1
        return self.classifier(hidden[:, -1, :]), calls


@torch.no_grad()
def accuracy(
    model: RecurrentRegisterStateTransformer,
    x: torch.Tensor,
    y: torch.Tensor,
    lengths: torch.Tensor,
    *,
...
def train_one(
    ...
    *,
    ...
    model = RecurrentRegisterStateTransformer(
        ...
        layers=layers,
        heads=heads,
        max_positions=max(24, ood_max_steps + recurrent_steps + 8),
    )
        ...
        for epoch in range(1, max_epochs_per_stage + 1):
            ...
            for bx, by, bl in loader:
                ...
                if not torch.isfinite(loss):
                    raise RuntimeError("non-finite register comparison loss")
                loss.backward()
                optimizer.step()
            ...
            if epoch % check_interval == 0 or epoch == max_epochs_per_stage:
                ...
                stable_checks = stable_checks + 1 if acc == 1.0 else 0
    ...
    if id_saturated:
        ...
        for steps, (x, y, lengths) in sampled.items():
            ood_by_steps[str(steps)] = accuracy(
                model,
                x,
                y,
                lengths,
...
def paired_analysis(results: list[Result]) -> dict:
    by_treatment = {
...
def main() -> None:
    ...
    payload = {
        ...
        "claim_boundary": (
            "Direct is lower-compute context. Serial-control and latent use matched "
            "K+1 forward calls and identical shared initialization/data/optimizer. "
            "OOD is emitted only after complete ID saturation."
        ),
    }
